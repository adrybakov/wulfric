...
* Fix bug in ``wulfric.io.dump_poscar``: only three atom positions were written. Now all
...
* Add ``with_time_reversal`` to ``wulfric.kpoints.get_path_and_points`` and
...
*   Major improvement of the plotting engine - focus on |plotly|_ as a backend.
...
behave differently in some edge cases. If you use them, a review is required.
